...
class WikiLinkProcessor:
    ...
    def __post_init__(self):
        ...
        self.regex_intext = re.compile(r"^#(\^[a-z0-9]{6})")

    def _intext_link(self, link: str, others: list[str]) -> tuple[str, list[str]]:
        cnt = 0
        for other in others:
            if link in other:
                cnt += 1
                other.replace(link, f'<a id="{link}"></a>')
        if cnt == 0:
            logger.warning(f"Link {link} not found in text.")
        elif cnt > 0:
            logger.warning(f"Link {link} found {cnt} times in text.")
    ...
    def __call__(self, fn: Path, content: str) -> str:
        ...
        if not self.regex.search(content):
            logger.info(f"No wiki link found in {fn}.")
            return content

        # 将content分成两部分，第一部分是其他内容，第二部分是图片链接
        parts = self.regex.split(content)
        others = parts[::2]
        wiki_links = parts[1::2]
        logger.info(f"Found {len(wiki_links)} obsidian-style image links in {fn}.")

        for wiki_link in wiki_links:
            intext_res = self.regex_intext.search(wiki_link)
            if intext_res:
                link, others = self._intext_link(intext_res.group(1), others)
                new_link = f'<a href="#{link}">{link}</a>'
                # TODO: 如果这里是公式的话，这里请示应该渲染成公式的编号，这
                # 很难进行处理！！！！
                continue
